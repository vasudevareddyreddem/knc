...
/* wishlist get api */
	...
	public function productdetails_get()
	{
		...
		//$product_details['imagepath']='https://order-organic.com/uploads/products/';
	...
	public function forgotpassword_post()
	{
		...
		if(count($forgotpasscheck)>0){
					...
					if($mobile!=''){		
						$msg=$six_digit_random_number.' is your Orderorganic verification code one-time use. Please DO NOT share this OTP with anyone to ensure account security';
						$ch = curl_init();
	...
	public function topoffers_get()
	{
		...
		if(count($top_offers)>0){
			...
			//$top_offers['path']='https://order-organic.com/uploads/productsimages/';
			$this->response($message, REST_Controller::HTTP_OK);	
	...
	public function dealsoftheday_get()
	{
		...
		if(count($deals)>0){
			...
			//$deals['path']='https://order-organic.com/uploads/productsimages/';
			$this->response($message, REST_Controller::HTTP_OK);	
	...
	public function seasonsales_get()
	{
		...
		if(count($ssales)>0){
			...
			//$ssales['path']='https://order-organic.com/uploads/productsimages/';
			$this->response($message, REST_Controller::HTTP_OK);	
	...
	public function trendigproducts_get()
	{
		...
		if(count($treding)>0){
			...
			//$treding['path']='https://order-organic.com/uploads/productsimages/';
			$this->response($message, REST_Controller::HTTP_OK);	
	...
	public function offersforyou_get()
	{
		...
		if(count($offers)>0){
			...
			//$offers['path']='https://order-organic.com/uploads/productsimages/';
			$this->response($message, REST_Controller::HTTP_OK);	
	...
	public function category_wise_search_post(){
			...
			/*delete old data*/
				...
				//echo '<pre>';prhttp://localhost/Orderorganic/api/customerApi/category_wise_leftside_filters?category_id=20int_r($idslist);
		...
		public function overallproducts_list_review_get(){
			...
			public function resendopt_post(){
					...
					if(count($forgotpasscheck)>0){
									...
									$username=$this->config->item('smsusername');
									$pass=$this->config->item('smspassword');
										$msg=' Your Orderorganic verification code is '.$six_digit_random_number;
										$ch = curl_init();
										curl_setopt($ch, CURLOPT_URL,"http://bhashsms.com/api/sendmsg.php");
										curl_setopt($ch, CURLOPT_POST, 1);
		...
		//echo '<pre>';print_r($cart_items);exit;
			...
			public function mobilehomebanners_get(){
				...
				if(count($bannerslist)>0){
					//$message = array('status'=>1,'path'=>'https://order-organic.com/assets/appbanners/','list'=>$bannerslist,'middlelist'=>$homepagemiddlebannerslist,'path1'=>'https://order-organic.com/assets/middlehomepagebanners/','message'=>'banners list are available');
