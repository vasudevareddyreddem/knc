...
class Bulkorders extends Admin_Controller {
	...
	public function __construct() {
		...
		$this->load->model('admin/bulkorders_model');
...

	{
		...
		$this->load->library('pagination');
		  ...
		  $config = [
		   'base_url'   => base_url('admin/bulkorders/index'),
		   'per_page'   => 10,
		   'total_rows'  => $this->bulkorders_model->count_by(array()),
		...
		$result=$this->bulkorders_model->limit($config['per_page'], $this->uri->segment(4) )->get_allorderdetails();

		//print_r($result);exit;

		$data['orderdata'] =  $result;

		$this->template->write_view('content', 'admin/bulkorders/index',$data);
...

	{
        ...
        $this->template->write_view('content', 'admin/bulkorders/add');
...
public function delete()
	...
	$result=$this->bulkorders_model->delete($id);
...
public function edit()

	{
		...
		$result=$this->bulkorders_model->geteditdata($id);
		...
		$this->template->write_view('content', 'admin/bulkorders/edit',$data);
...

	{
		...
		$result=$this->bulkorders_model->getviewdata($id);
		...
		$this->template->write_view('content', 'admin/bulkorders/view',$data);
...
public function updatedorder()

	{
     ...
     // print_r( $data);exit;
       ...
       $res=$this->bulkorders_model->update($id,$data);
...
public function search()

	{
		...
		$result=$this->bulkorders_model->limit($config['per_page'], $this->uri->segment(4) )->search();
