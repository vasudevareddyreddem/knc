...
public function get_all_subitem_list($catid,$subcatid)
 ...
 public function get_subitem_list($subitem_id,$item_id)
	...
	/*itemwise*/
	public function get_item_all_brand_list($itemid,$where){
		$this->db->select('products.'.$where)->from('products');
		$this->db->where('itemwise_id',$itemid);
		$this->db->where('item_status',1);
		$this->db->where('products.'.$where.' !=','');
		$this->db->group_by($where);
		return $this->db->get()->result_array();
	}
	public function get_item_all_price_list($itemid){
		$this->db->select('products.item_cost,products.special_price,products.offer_expairdate')->from('products');
		$this->db->where('itemwise_id',$itemid);
		$this->db->where('item_status',1);
		$this->db->where('item_cost!=','');
	...
	public function get_all_itemwiseproducts_list($itemid){
		...
		$this->db->join('sub_items', 'sub_items.subitem_id =products.subitemid', 'left');	
		$this->db->where('products.item_status',1);
		$this->db->where('products.itemwise_id',$itemid);
		return $this->db->get()->result_array();
	}
	public function item_details($itemid){
		$this->db->select('items_list.item_name')->from('items_list');
		$this->db->where('id',$itemid);
		$this->db->where('status',1);
		return $this->db->get()->row_array();
	}
	public function get_subitem_details($subitem_id){
		$this->db->select('sub_items.subitem_name')->from('sub_items');
		$this->db->where('subitem_id',$subitem_id);
		$this->db->where('status',1);
		return $this->db->get()->row_array();
	}
	public function get_subcategory_details($subcat_id){
		$this->db->select('subcategories.subcategory_name')->from('subcategories');
		$this->db->where('subcategory_id',$subcat_id);
		$this->db->where('status',1);
		return $this->db->get()->row_array();
	}
	public function save_itemsearchdata($data){
		
		$this->db->insert('item_wise_filter_search', $data);
		return $insert_id = $this->db->insert_id();
	}
	public function get_item_all_previous_search_fields()
	{
		$this->db->select('*')->from('item_wise_filter_search');
		$this->db->where('Ip_address',$this->input->ip_address());
		return $this->db->get()->result_array();
	}
	public function item_wise_update_deails($id,$data){
		$this->db->where('id', $id);
		return $this->db->update('item_wise_filter_search', $data);
	}
	public function get_itemwise_data_item_id($ip)
	{
		$this->db->select('item_wise_filter_search.itemid')->from('item_wise_filter_search');
		$this->db->where('ip_address',$ip);
		return $this->db->get()->row_array();
	}
	public function get_all_previous_itemwise_search_fields($ip)
	{
		$this->db->select('*')->from('item_wise_filter_search');
		$this->db->where('Ip_address',$ip);
		return $this->db->get()->result_array();
	...
	$this->db->group_by('item_wise_filter_search.colour');
	$this->db->group_by('item_wise_filter_search.size');
	$this->db->group_by('item_wise_filter_search.ram');
	$this->db->group_by('item_wise_filter_search.os');
	$this->db->group_by('item_wise_filter_search.sim_type');
	$this->db->group_by('item_wise_filter_search.camera');
	$this->db->group_by('item_wise_filter_search.internal_memeory');
	$this->db->group_by('item_wise_filter_search.screen_size');
	$this->db->group_by('item_wise_filter_search.Processor');
	$this->db->group_by('item_wise_filter_search.printer_type');
	$this->db->group_by('item_wise_filter_search.type');
	$this->db->group_by('item_wise_filter_search.max_copies');
	$this->db->group_by('item_wise_filter_search.paper_size');
	$this->db->group_by('item_wise_filter_search.headphone_jack');
	$this->db->group_by('item_wise_filter_search.noise_reduction');
	$this->db->group_by('item_wise_filter_search.usb_port');
	$this->db->group_by('item_wise_filter_search.compatible_for');
	$this->db->group_by('item_wise_filter_search.scanner_type');
	$this->db->group_by('item_wise_filter_search.resolution');
	$this->db->group_by('item_wise_filter_search.f_stop');
	$this->db->group_by('item_wise_filter_search.minimum_focusing_distance');
	$this->db->group_by('item_wise_filter_search.aperture_withmaxfocal_length');
	$this->db->group_by('item_wise_filter_search.picture_angle');
	$this->db->group_by('item_wise_filter_search.weight');
	$this->db->group_by('item_wise_filter_search.occasion');
	$this->db->group_by('item_wise_filter_search.material');
	$this->db->group_by('item_wise_filter_search.collar_type');
	$this->db->group_by('item_wise_filter_search.gender');
	$this->db->group_by('item_wise_filter_search.sleeve');
	$this->db->group_by('item_wise_filter_search.look');
	$this->db->group_by('item_wise_filter_search.style_code');
	$this->db->group_by('item_wise_filter_search.inner_material');
	$this->db->group_by('item_wise_filter_search.waterproof');
	$this->db->where('ip_address',$ip);
	$query=$this->db->get()->result_array();
	//echo '<pre>';print_r($query);exit;
		...
		$return['filterslist'] = $this->get_itemwise_filters_search($itemid,$minamount,$maxamount,$offer,$brand,$discount,$colour,$size,$ram,$os,$sim_type,$camera,$internal_memeory,$screen_size,$Processor,$printer_type,$type,$max_copies,$paper_size,$headphone_jack,$noise_reduction,$usb_port,$compatible_for,$scanner_type,$resolution,$f_stop,$minimum_focusing_distance,$aperture_withmaxfocal_length,$picture_angle,$weight,$occasion,$material,$collar_type,$gender,$sleeve,$look,$style_code,$inner_material,$waterproof);
		//echo $this->db->last_query();exit;
		//echo '<pre>';print_r($return['filterslist']);exit;
		if(!empty($return['filterslist']))
		{
	...
	public function get_itemwise_filters_search($itemid,$minamount,$maxamount,$offer,$brand,$discount,$colour,$size,$ram,$os,$sim_type,$camera,$internal_memeory,$screen_size,$Processor,$printer_type,$type,$max_copies,$paper_size,$headphone_jack,$noise_reduction,$usb_port,$compatible_for,$scanner_type,$resolution,$f_stop,$minimum_focusing_distance,$aperture_withmaxfocal_length,$picture_angle,$weight,$occasion,$material,$collar_type,$gender,$sleeve,$look,$style_code,$inner_material,$waterproof){
	...
	public function get_groupwise_product_list($catid,$brand,$min,$max){
		$this->db->select('products.item_id,products.category_id,products.subcategory_id,products.subitemid,products.itemwise_id,products.item_name,products.item_status,products.item_cost,products.special_price,products.item_quantity,products.offer_percentage,products.offer_amount,products.offer_expairdate,products.offer_type,products.discount,products.offers,products.item_image')->from('products');
		$amtwhere='products.item_cost BETWEEN '.'"'.$min.'"'.' AND '.$max;
		$this->db->where($amtwhere);
		$this->db->where('category_id',$catid);
		$this->db->where('brand',$brand);
		$this->db->where('item_status',1);
	...
	public function get_group_all_offer_list($catid,$brand,$min,$max){
		$sql = "SELECT offer_percentage, offers, offer_expairdate,brand  FROM `products` WHERE `category_id` = '".$catid."' AND brand = '".$brand."' AND item_cost BETWEEN '".$min."' AND '".$max."'  AND item_status = 1";
		return $this->db->query($sql)->result_array();
	}
	...
	public function get_category_all_previous_search_fields()
	{
		$this->db->select('*')->from('brandwise_filters');
		$this->db->where('Ip_address',$this->input->ip_address());
		return $this->db->get()->result_array();
	}
	public function category_wise_update_deails($id,$data){
		$this->db->where('id', $id);
		return $this->db->update('brandwise_filters', $data);
	}
	public function save_categorysearchdata($data){
		
		$this->db->insert('brandwise_filters', $data);
		return $insert_id = $this->db->insert_id();
	}
	public function get_categorywise_search_result_data($ip)
	{
	$this->db->select('brandwise_filters.*')->from('brandwise_filters');
	$this->db->group_by('brandwise_filters.offer');
	$this->db->group_by('brandwise_filters.brand');
	$this->db->group_by('brandwise_filters.discount');
	$this->db->group_by('brandwise_filters.colour');
	$this->db->group_by('brandwise_filters.size');
	$this->db->group_by('brandwise_filters.ram');
	$this->db->group_by('brandwise_filters.os');
	$this->db->group_by('brandwise_filters.sim_type');
	$this->db->group_by('brandwise_filters.camera');
	$this->db->group_by('brandwise_filters.internal_memeory');
	$this->db->group_by('brandwise_filters.screen_size');
	$this->db->group_by('brandwise_filters.Processor');
	$this->db->group_by('brandwise_filters.printer_type');
	$this->db->group_by('brandwise_filters.type');
	$this->db->group_by('brandwise_filters.max_copies');
	$this->db->group_by('brandwise_filters.paper_size');
	$this->db->group_by('brandwise_filters.headphone_jack');
	$this->db->group_by('brandwise_filters.noise_reduction');
	$this->db->group_by('brandwise_filters.usb_port');
	$this->db->group_by('brandwise_filters.compatible_for');
	$this->db->group_by('brandwise_filters.scanner_type');
	$this->db->group_by('brandwise_filters.resolution');
	$this->db->group_by('brandwise_filters.f_stop');
	$this->db->group_by('brandwise_filters.minimum_focusing_distance');
	$this->db->group_by('brandwise_filters.aperture_withmaxfocal_length');
	$this->db->group_by('brandwise_filters.picture_angle');
	$this->db->group_by('brandwise_filters.weight');
	$this->db->group_by('brandwise_filters.occasion');
	$this->db->group_by('brandwise_filters.material');
	$this->db->group_by('brandwise_filters.collar_type');
	$this->db->group_by('brandwise_filters.gender');
	$this->db->group_by('brandwise_filters.sleeve');
	$this->db->group_by('brandwise_filters.look');
	$this->db->group_by('brandwise_filters.style_code');
	$this->db->group_by('brandwise_filters.inner_material');
	$this->db->group_by('brandwise_filters.waterproof');
	$this->db->where('ip_address',$ip);
	$query=$this->db->get()->result_array();
	//echo '<pre>';print_r($query);exit;
		...
		$return['filterslist'] = $this->get_categorywise_filters_search($group,$category_id,$minamount,$maxamount,$offer,$brand,$discount,$colour,$size,$ram,$os,$sim_type,$camera,$internal_memeory,$screen_size,$Processor,$printer_type,$type,$max_copies,$paper_size,$headphone_jack,$noise_reduction,$usb_port,$compatible_for,$scanner_type,$resolution,$f_stop,$minimum_focusing_distance,$aperture_withmaxfocal_length,$picture_angle,$weight,$occasion,$material,$collar_type,$gender,$sleeve,$look,$style_code,$inner_material,$waterproof);
	...
	public function get_categorywise_filters_search($group,$category_id,$minamount,$maxamount,$offer,$brand,$discount,$colour,$size,$ram,$os,$sim_type,$camera,$internal_memeory,$screen_size,$Processor,$printer_type,$type,$max_copies,$paper_size,$headphone_jack,$noise_reduction,$usb_port,$compatible_for,$scanner_type,$resolution,$f_stop,$minimum_focusing_distance,$aperture_withmaxfocal_length,$picture_angle,$weight,$occasion,$material,$collar_type,$gender,$sleeve,$look,$style_code,$inner_material,$waterproof){
		...
		$this->db->where('brand',$group);
		
		return $this->db->get()->result_array();
	}

	public function get_categorywise_data_item_id($ip)
	{
		$this->db->select('brandwise_filters.category_id,brandwise_filters.group,brandwise_filters.max,brandwise_filters.min')->from('brandwise_filters');
		$this->db->where('ip_address',$ip);
		return $this->db->get()->row_array();
	}
	public function get_categoryprivous_data($ip)
	{
		$this->db->select('*')->from('brandwise_filters');
		$this->db->where('ip_address',$ip);
		return $this->db->get()->result_array();
	}
	public function delete_previous_data($id)
	{
	...
	public function get_groupwise_all_price_list_minmax($catid,$brand){
