...
<!-- Filter Sidebar -->
<span id="unitwisechangeitemids<?php echo time(); ?>">
   <?php if(count($subcategory_porduct_list)>0) { ?>
   <?php
      $customerdetails=$this->session->userdata('userdetails');
      $cnt=0;$cnt2=101;foreach ($subcategory_porduct_list as $productslist){ 
               ...
               <div class="col-md-6">
				...
				<?php if(isset($productslist['unitproducts_list']) && count($productslist['unitproducts_list'])>0){ ?>                 
				 <p class="">Available in: &nbsp;&nbsp;
                     ...
                     <span onclick="getunitwiseproductsinunit('<?php echo $list['item_id']; ?>','<?php echo $cnt; ?>','<?php echo time(); ?>')" class="btn_cus btn_cus_acti pad_rig"><?php echo $list['unit']; ?></span>&nbsp;&nbsp;
                     <?php }else{ ?>
                     <span onclick="getunitwiseproductsinunit('<?php echo $list['item_id']; ?>','<?php echo $cnt; ?>','<?php echo time(); ?>')" class="btn_cus pad_rig"><?php echo $list['unit']; ?></span>&nbsp;&nbsp;
                     <?php  } ?>
                     <?php } ?>
                  </p>
				<?php } ?>
                  <div  class="input-group incr_btn pull-left">
                     ...
                     <button style="width:20px;padding:6px;"type="button" onclick="productqty1('<?php echo $cnt; ?>','<?php echo time(); ?>');" class="btn btn-primary btn-number btn-small"  data-type="minus" data-field="quant[2]">
                     <span style="margin:-4px" class="glyphicon glyphicon-minus"></span>
                     </button>
                     </span>
                     <input type="text" name="qty" id="qty1<?php echo $cnt; ?><?php echo time(); ?>" readonly  class="form-control input-number" value="1" min="1" max="<?php echo $productslist['item_quantity']; ?>">
                     <span class="input-group-btn">
                     <button style="width:20px;padding:6px" onclick="productqtyincreae1('<?php echo $cnt; ?>','<?php echo time(); ?>');" type="button" class="btn btn-primary btn-number btn-small" data-type="plus" data-field="quant[2]">
                  ...
                  <div class="pull-right">
                     ...
                     <a href="javascript:void(0);" onclick="unitaddwhishlidtss('<?php echo $productslist['item_id']; ?>','<?php echo $cnt; ?>','<?php echo time(); ?>');" id="addwhish<?php echo $productslist['item_id']; ?><?php echo $cnt; ?><?php echo time(); ?>"  ><span id="addwishlistids<?php echo $productslist['item_id']; ?><?php echo $cnt; ?><?php echo time(); ?>" class="btn btn-primary btn-sm ">Add to Whishlist</span></a> 
                     <?php }else{ ?>	
                     <a href="javascript:void(0);" onclick="unitaddwhishlidtss('<?php echo $productslist['item_id']; ?>','<?php echo $cnt; ?>','<?php echo time(); ?>');" id="addwhish<?php echo $productslist['item_id']; ?><?php echo $cnt; ?><?php echo time(); ?>"  ><span id="addwishlistids<?php echo $productslist['item_id']; ?><?php echo $cnt; ?><?php echo time(); ?>" class="btn btn-primary btn-sm wishadd">Add to Whishlist</span></a> 
                     <?php } ?>
...
<script>
   function unitaddwhishlidtss(id,val,time){
   jQuery.ajax({
   ...
   function getunitwiseproductsinunit(itemid,cnt,time){
   if(itemid!=''){
    jQuery.ajax({
   	...
   	dataType: 'html',
   	success: function (data) {
   			$("#unitwisechangeitemids"+time).empty();
   			$("#unitwisechangeitemids"+time).append(data);
   	}
   });
   
   }
   }
   function productqty1(id,anid){
   ...
   $('#qty1'+id+anid).val(qty);
   }else{
   $('#qty1'+id+anid).val(qty - 1);
   $("#qtymesage"+id).html('');
   jQuery.ajax({
   ...
   function productqtyincreae1(id,anid){
   var pid=document.getElementById("product_id1"+id).value;
